...
def test_take_with_negative_count_uses_python_slice_semantics() -> None:
    assert ArrayList([1, 2, 3, 4]).take(-1).to_py_list() == [1, 2, 3]
...
def test_index_of_returns_exact_position() -> None:
    assert ArrayList([3, 1, 2]).index_of(2) == 2


def test_index_of_raises_for_missing_item() -> None:
    with pytest.raises(ValueError):
        ArrayList([1, 2, 3]).index_of(4)


def test_map_returns_transformed_list() -> None:
    assert ArrayList([3, 1, 2]).map(lambda item: item * 2).to_py_list() == [6, 2, 4]


def test_reduce_returns_none_for_empty_list() -> None:
    assert ArrayList[int]().reduce(lambda left, right: left + right) is None


def test_reduce_folds_values_left_to_right() -> None:
    assert ArrayList([3, 1, 2]).reduce(lambda left, right: left - right) == 0


def test_filter_keeps_only_matching_items() -> None:
    assert ArrayList([3, 1, 2, 4]).filter(lambda item: item % 2 == 0).to_py_list() == [2, 4]


def test_order_sorts_using_key_selector() -> None:
    assert ArrayList([3, 1, 2, 4]).order(lambda item: item).to_py_list() == [1, 2, 3, 4]


def test_slice_returns_new_list() -> None:
    assert ArrayList([3, 1, 2, 4])[1:3].to_py_list() == [1, 2]
